 ...
 const handleSubmit = async (e: FormEvent) => {
   ...
   if (savedUserJson) {
     try {
       ...
       // Verifica se o e-mail e a senha digitados batem com o cadastro do localStorage
       if (
         email.toLowerCase().trim() === savedUser.email.toLowerCase().trim() &&
   ...
   // 3️⃣ Se não bateu com nenhum dos dois, dá erro
   setLoading(false);
...
<RegisterForm
               ...
               onSuccess={(session) => {
                 // Salva no formato puro esperado pelo login
                 if (session && session.user) {
                   localStorage.setItem('registered_user', JSON.stringify({
                     email: session.user.email,
                     password: password || '12345', // garante o fallback caso o mock omita
