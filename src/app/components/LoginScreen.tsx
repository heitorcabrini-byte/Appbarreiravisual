...
function LoginForm({ highContrastMode, onSuccess }: LoginFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    let contaLocal: any = null;

    try {
      if (savedUserJson) {
        contaLocal = JSON.parse(savedUserJson);
      } else if (savedSessionJson) {
        const sessionObj = JSON.parse(savedSessionJson);
        if (sessionObj && sessionObj.user) {
          contaLocal = sessionObj.user;
        }
      }
    } catch (err) {
